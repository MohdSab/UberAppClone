...
class Dispatcher:
    ...
    def request_driver(self, passenger: Passenger) -> Optional[Driver]:
        ...
        if len(self._drivers) == 0:
            ...
            list_drivers = list(self._drivers.values())
            list_drivers_idle = []

            for driver in list_drivers:
                if driver.is_idle:
                    list_drivers_idle.append(driver)

            if len(list_drivers_idle) > 0:
                best_time = list_drivers_idle[0].get_travel_time(
                    passenger.origin)
                best_driver = list_drivers_idle[0]

            else:
                best_time = 0
                best_driver = None

            for driver in list_drivers_idle:
                if driver.get_travel_time(passenger.origin) < best_time:
                    best_driver = driver
                    best_time = driver.get_travel_time(passenger.origin)
    ...
    def request_passenger(self, driver: Driver) -> Optional[Passenger]:
        ...
        """
        # TO-DO
        list_drivers_keys = list(self._drivers.keys())
        if driver.id not in list_drivers_keys:
            self._drivers[driver.id] = driver

        if len(self._waiting_passengers) == 0:
            return None

        else:
            return self._waiting_passengers.pop(0)
