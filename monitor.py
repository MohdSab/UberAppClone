...
class Monitor:
    ...
    def _average_total_distance(self) -> float:
        ...
        # TO-DO
        travel_distance = 0
        count = len(self._activities[DRIVER].keys())
        for value in self._activities[DRIVER].values():
            if len(value) >= 2:
                for i in range(len(value) - 1):
                    curr = value[i]
                    stop = value[i + 1]
                    travel_distance += manhattan_distance(curr.location,
                                                          stop.location)
        if count == 0:
            return 0
        else:
            return travel_distance / count
    ...
    def _average_trip_distance(self) -> float:
        ...
        for k in range(len(needed) - 1):
            if needed[k].description == PICKUP:
                curr = needed[k]
                stop = needed[k + 1]
                travel_distance += manhattan_distance(curr.location,
                                                      stop.location)
