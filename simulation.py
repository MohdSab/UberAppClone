...
class Simulation:
    ...
    #     A sequence of original events arranged in priority determined
    #     by the event
    #     sorting order.
    ...
    def run(self, initial_events: List[Event]) -> Dict[str, float]:
        """Run the simulation on the list of original events
        in <initial_events>.

        Return a dictionary containing statistics of the simulation,
        according to the specifications in the assignment handout.

        initial_events: An initial list of original events.
        """
        # TO-DO

        # Add all initial original events to the event queue.

        # Until there are no more original events, remove an event
        # from the event queue and do it. Add any returned
        # original events to the event queue.

        for event in initial_events:
            self._events.add(event)

        while not self._events.is_empty():
            action = self._events.remove()
            doing = action.do(self._dispatcher, self._monitor)
            for final in doing:
                self._events.add(final)
